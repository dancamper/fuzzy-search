               ...
               (let ((result hit-metadata))
                 (loop :for (k v) :on search-metadata :by #'cddr
                       :do (cond ((eql k :confidence)
                                 ...
                                 ((eql k :edit-distance)
                                  (setf (getf result :edit-distance) (max (getf result :edit-distance 0) v)))
                                 ((not (getf result k))
             ...
             (fixup-metadata (metadata)
               (let* ((m (copy-list metadata))
                 ...
                 (multiple-value-bind (match-start match-end) (ppcre:scan field-name-scanner type-str)
                   (when match-start
                     (setf (getf m :field) (str:substring match-start match-end type-str))))
                 m))
             (score-metadata (metadata)
               (let ((conf (getf metadata :confidence)))
                 (setf conf (/ conf (getf metadata :num-words 1)))
                 conf)))
      (let* ((class-symbol (class-symbol-from-value-type type))
              ...
              :do (let ((candidates (gethash (hash-value item) *test-hash-results*)))
                    (when candidates
                      (loop :for candidate :in candidates
                            :do (push (merge-metadata candidate (metadata item)) (gethash (getf candidate :entity-id) search-results))))))
        ...
        (loop :for entity-id :being :the :hash-keys :in search-results :using (hash-value hits)
              :do (let ((score (reduce #'+ hits :key #'score-metadata)))
